...
平台由 **八条产品线 + 一个工程底座** 组成，各产品线共享同一套可插拔 AI 基础设施：
...
| 👥 **客户管理（CRM）** | 客户档案（公司 / 联系人 / A-D 分级 / 来源 / 标签）、跟进记录（电话 / 邮件 / 会议）、即将跟进提醒 | 多租户隔离、客户统计概览（分级 / 来源分布）、跟进时间线 |
| 📦 **商品管理** | 商品档案（SKU / 分类 / 成本售价 / 库存）、分类列表、统计概览 | 与亚马逊联动：绑定 ASIN / 站点，为订单分析与文案生成提供商品上下文 |
| 🔌 **平台对接** | 亚马逊 SP-API：OAuth 授权、订单 / 商品目录 / FBA 库存 / 报表，覆盖 10 大站点（US/UK/DE/JP 等） | **AI 联动闭环**：订单智能分析、热销商品自动生成文案、客服订单上下文、商品描述批量翻译；**内置 Mock 模式**（`AMAZON_MOCK_MODE`），无需真实凭证即可跑通授权与数据全链路（真实凭证联调待接入） |
...
数据模型层   向量库 | 知识库 | LLM(多端点故障转移) | 术语库/翻译记忆 | 客户/跟进/商品档案
平台对接层   亚马逊 SP-API（订单/目录/库存/报表，Mock 可演示）→ AI 联动
```
...
- 亚马逊对接默认 Mock 模式（`AMAZON_MOCK_MODE=True`），无需凭证即可体验授权 / 订单 / 库存 / 报表全链路；真实联调需在 `backend/.env` 配置 LWA + AWS IAM 凭证并关闭 Mock，详见 [`docs/操作手册.md`](docs/操作手册.md) 第四章
...
| [`docs/开发手册/`](docs/开发手册/) | 开发手册：知识库建设关键节点日志（时间线 + 踩坑对照 + 复现命令） |
...
```
├── backend/                 # FastAPI 后端
│   ├── app/api/v1/          # REST API（客服 / 文案 / 翻译 / 知识库 / 洞察 / CRM / 商品 + 亚马逊 SP-API + 鉴权）
│   ├── app/core/            # LLM / Embedding / 向量库 / 安全 / 亚马逊客户端
│   ├── app/services/        # RAG / 客服 / 文案 / 翻译 / 洞察 / CRM / 商品 / 亚马逊 AI 联动
│   ├── app/db/              # SQLAlchemy 模型（11 张表）+ Alembic 迁移
│   ├── tests/               # 单元 / 冒烟 / 租户隔离测试
│   └── scripts/             # ingest / eval_rag / check_config
├── frontend/                # Vue 3 + Element Plus（九大功能页面 + 登录）
├── docs/                    # 项目文档 / 操作手册 / 升级推进 / 开发手册 / 简历面试
├── launcher.py              # 一键启动器
