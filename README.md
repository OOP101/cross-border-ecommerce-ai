...
**1. 混合检索体系**
向量 + BM25 双路召回 + RRF 融合 + 可插拔重排序；BM25 索引按语料指纹缓存、向量归一化矩阵惰性缓存，消除每次查询的全量重建开销。

**2. 术语库 + 两级翻译记忆**
持久化术语库注入 Prompt 强约束专业译法，30+ 语种译法一致；「进程内 LRU + SQLite 持久化」两级缓存，重复文本零模型调用成本。

**3. 四层架构 + 可插拔 Mock**
交互 / 编排 / RAG 检索 / 数据模型四层与六大模块 REST API；LLM / Embedding / 向量库抽象层默认 Mock，无 API Key 跑通全链路，改配置即切真实模型。

**4. 客服 Agent 与工程化**
意图识别、多轮上下文、高风险自动转人工；`asyncio.gather` 多段文案并发生成；PBKDF2 + JWT 鉴权、Docker 一键启动，自动化测试覆盖检索 / 客服 / 鉴权核心链路。
...
| # | 阶段 | 说明 |
|:--|:--|:--|
| 1 | 语料接入 | 商品信息 / 客服语料，PII 脱敏 |
| 2 | 分块与索引缓存 | 语义分块；BM25 指纹缓存 / 向量矩阵惰性缓存 |
| 3 | 多语言文案生成 | 术语库注入 Prompt 强约束 · `asyncio` 并发 |
| 4 | 两级翻译记忆 | 进程内 LRU + SQLite，重复文本零模型调用 |

**在线 · 客服与评测链路**

| # | 阶段 | 说明 |
|:--|:--|:--|
| 1 | 混合检索 | 向量 + BM25 + RRF + 可插拔重排序 |
| 2 | 智能客服问答 | 多轮上下文 · SSE 流式逐字输出 |
| 3 | 高风险转人工 | 投诉 / 退款 / 法律，来源可溯源 |
| 4 | 评测回归迭代 | 52 条评测集 · Recall@3 100% · MRR 0.942 |

## 后续规划

| 方向 | 现状 → 规划 |
|:--|:--|
| 评测集扩容 | 现状：52 条评测集当前全命中。规划：向多品类、多站点扩容并建立品类级基线，防止指标过拟合单一语料。 |
| 术语资产版本化 | 规划：术语库 / 话术库版本管理与变更审批，跨语种一致性核查自动化，资产可沉淀可回滚。 |
| 评测回归接入 CI | 规划：检索参数 / Prompt 变更自动跑批，输出 Recall@3 / MRR 优化前后对比报告，回归不达标不上线。 |
| 客服知识运营 | 规划：高风险问题库持续扩充，转人工边界与兜底话术按真实会话回流迭代，判断权始终留给人。 |
